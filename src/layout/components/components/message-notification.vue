<template>
            ...
            <a-list-item-meta :title="item.title">
              <template #description>
                <div class="flex justify-between" style="font-size: 13px;">
                  <span>{{ $t('system.sender') }}：{{item.send_user.nickname}}</span>
                  <span>{{ $t('system.time') }}：{{item.created_at.substr(0, 10)}}</span>
                </div>
              </template>
              <template #avatar>
  ...
  <a-modal v-model:visible="detailVisible" :footer="false">
    <template #title>{{ $t('system.messageDetail') }}</template>
    <a-typography :style="{ marginTop: '-30px' }">
      ...
      <a-typography-paragraph class="text-right" style="font-size: 13px; color: var(--color-text-3)">
        <a-space size="large">
          <span>{{ $t('system.createTime') }}：{{ row?.created_at }}</span>
        </a-space>
