...
<template>
  <div class="sider customer-scrollbar flex flex-col items-center bg-gray-800 dark:border-blackgray-5">
    <div class="columns-logo">
      <div class="columns-logo-icon"><icon-robot :size="18" /></div>
    </div>
...
<style>
.columns-logo { padding: 12px 0 8px; display: flex; justify-content: center; }
.columns-logo-icon {
  width: 36px; height: 36px; background: var(--color-primary); border-radius: var(--radius-md);
  display: flex; align-items: center; justify-content: center; color: var(--color-on-primary);
}
html.dark .columns-logo-icon { background: var(--color-inverse-primary); color: var(--color-on-primary-fixed); }
