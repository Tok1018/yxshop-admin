  ...
  <div class="sys-search-container">
    <div class="ssc-bg">
      <div class="w-6/12 mx-auto center-box">
        <div class="mt-10"><img :src="`${$url}logo.png`" width="100" class="mx-auto" alt="Logo"/></div>
        <div class="mt-10">
          <a-input
            size="large"
            ref="searchInputRef"
        ...
        <div class="mt-5">
          <a-space size="large" class="flex justify-center">
            <a-space><a-tag>{{ $t('system.searchHelp1') }}</a-tag><a-tag>{{ $t('system.searchHelp') }}</a-tag></a-space>
            <a-space><a-tag><icon-caret-up /></a-tag><a-tag><icon-caret-down /></a-tag><a-tag>{{ $t('system.searchHelp4') }}</a-tag></a-space>
            <a-space><a-tag>{{ $t('system.searchHelp2') }}</a-tag><a-tag>{{ $t('system.searchHelp5') }}</a-tag></a-space>
            <a-space><a-tag>{{ $t('system.searchHelp3') }}</a-tag><a-tag>{{ $t('system.searchHelp6') }}</a-tag></a-space>
          </a-space>
        </div>
          ...
          <template v-for="res in resultList">
            ...
            >
              <div class="icon-box flex justify-center items-center">
                <sa-icon v-if="res.meta.icon" :icon="res.meta.icon" />
                <icon-menu v-else />
              </div>
