<template>
  <span
    class="mp-icon"
    :style="{ width: size + 'px', height: size + 'px', backgroundImage: bgImage, display: bgImage ? 'inline-block' : 'none' }"
  ></span>
</template>
...
const bgImage = computed(() => {
  if (!props.name || !ICON_PATHS[props.name]) return ''
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${props.size}" height="${props.size}" viewBox="0 0 24 24" fill="none" stroke="${props.color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${ICON_PATHS[props.name]}</svg>`
