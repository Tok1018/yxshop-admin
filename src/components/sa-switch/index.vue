...
const props = defineProps({
  ...
  valType: { type: String, default: 'auto' },
...
watch(
  ...
  (vl) => {
    if (props.valType === 'string') {
      ...
      value.value = Number(vl)
    } else {
      // 'auto' mode: preserve original type (number wins if it looks like a number)
      value.value = (vl === 0 || vl === 1 || vl === '0' || vl === '1') ? Number(vl) : vl
    }
  },
  { immediate: true }
)
