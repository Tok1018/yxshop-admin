        ...
        <a-row style="margin-bottom: 15px" v-if="!options.pageSimple">
          <a-col :span="16">
            <a-space>
              ...
              <a-button v-if="options.move.show" v-auth="options.move.auth || []" @click="moveMultipleAction">
                <template #icon> <icon-drag-arrow /> </template> {{ options.move.text || '移动' }}
              </a-button>
                          ...
                          <template #cell="{ record, column, rowIndex }">
                            ...
                            <template v-else-if="rowChild.type === 'image'">
                              <a-avatar @click="imageSee(rowChild, record, rowChild.dataIndex)" :size="row.size || 64" shape="square">
                                <img :src="imageView(record[rowChild.dataIndex])" style="object-fit: contain; cursor: pointer" alt="图片" />
                              </a-avatar>
                            </template>
                    ...
                    <template #cell="{ record, column, rowIndex }">
                      ...
                      <template v-else-if="row.type === 'image'">
                        <a-avatar @click="imageSee(row, record, row.dataIndex)" :size="row.size || 64" shape="square">
                          <img :src="imageView(record[row.dataIndex])" style="object-fit: contain; cursor: pointer" alt="图片" />
                        </a-avatar>
                      </template>
...
const moveAction = async (record) => {
  const params = { ids: [record[options.value.pk]] }
  if (options.value.move.func && isFunction(options.value.move.func)) {
    options.value.move.func(params)
  } else {
    console.error(`sa-table error：crud.move.func is not Function.`)
  }
}

// 批量移动
const moveMultipleAction = async () => {
  const params = { ids: selecteds.value }
  if (selecteds.value && selecteds.value.length > 0) {
    if (options.value.move.func && isFunction(options.value.move.func)) {
      options.value.move.func(params)
